...
class ADNIDataset2D(Dataset):
    def __init__(
        self, root="data\\ADNI1\\ADNI1-Screening-Nifti-2D", domain=None, split=None, transform=None
    ) -> None:
    ...
    def load_image(self, index: int):
        ...
        return resize(image, (224, 224), mode="constant")
    ...
    def __getitem__(self, index: int) -> Tuple[torch.Tensor, int]:
        ...
        if self.transform:
            return self.transform(sample)  # return data, label (X, y)
        else:
